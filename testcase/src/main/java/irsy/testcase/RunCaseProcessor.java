...
public class RunCaseProcessor {
	...
	public void preTestProcess(){
		...
		initSystemParams();
		cleanDir(downloadPath);
		cleanDir("./testScreenshots");
		for(Cookie cookie : Login2Service.getSPCookies()){
			driver.manage().addCookie(cookie);
		}
		//确保受理机构下的那个渠道是审核通过、激活成功的 ,并且添加了最少一种支付方式
		if(!ChannelService.checkAddChannelPreConditionAndProcess() && !ChannelPCService.checkUniqueInnerChannelPC())
			throw new SwiftPassException("检查受理机构及其子渠道", "失败！");
		driver.get(spServer + homePath);
		logger.info("非登录测试的初始化工作完成，当前在威富通登录后的主页上...");*/
