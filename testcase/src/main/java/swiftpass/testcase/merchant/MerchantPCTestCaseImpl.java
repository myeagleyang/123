...
public class MerchantPCTestCaseImpl extends TestCaseImpl implements MerchantPCTestCase{
	public MerchantPCTestCaseImpl(WebDriver driver) {
		super(driver);
	}
	...
	public void addMerchantPC(HashMap<String, String> params) {
		...
		try{
			...
			page.clickAddPT();
			//支付中心为弹框类型
//			if(!StringUtils.isEmpty(errorMsg) && !errorMsg.contains("商户支付类型配置已存在")
//					&& !errorMsg.equals("请先选择支付通道") && isParentChnSetPC.equals("true")){
...
//			}
			...
			//支付中心为下拉框类型
			if(!StringUtils.isEmpty(errorMsg) && !errorMsg.contains("商户支付类型配置已存在")
					&& !errorMsg.equals("请先选择支付中心") && isParentChnSetPC.equals("true")){
				pcAddPage.directSelectPayType(isSelectPT, payTypeName, payTypeId);
			}else{
...
//		}
	...
	public void runDetailPC(MerchantPCDetailCaseBean caseBean) {
		logger.info("本次商户管理——商户支付类型配置详情测试使用的测试数据是: " + caseBean);
		//	获取测试数据参数
		String isClickFirstRowMerchant = caseBean.getIsClickFirstRowMerchant();
		String isClickFirstRowPT = caseBean.getIsClickFirstRowPT();
